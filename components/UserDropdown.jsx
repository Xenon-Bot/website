...
export default function UserDropdown({user, above = true}) {
    ...
    return (
        <div className="relative">
            ...
            <div className={`${above ? 'absolute': ''} bg-theme-light shadow-xl rounded-md right-0 mt-3 flex flex-col ${visible ? '' : 'hidden'}`} onClick={() => setVisible(false)}>
                <Link href={`/users/${user.id}`} passHref>
                    <a className="flex items-center px-4 pt-2 pb-1 rounded-t-md hover:bg-theme-dark">
                        <FontAwesomeIcon icon={faUser} className="mr-2 text-gray-400"/>
                        <div className="text-lg whitespace-nowrap">Profile</div>
                    </a>
                </Link>
                <Link href="/templates/add" passHref>
                    <a className="flex items-center px-4 py-2 rounded-t-md hover:bg-theme-dark">
                        ...
                        <div className="text-lg whitespace-nowrap">Add Template</div>
                    </a>
                </Link>
                ...
                <Link href="/logout" passHref>
                    <a className="flex items-center px-4 pt-1 pb-2 rounded-b-md hover:bg-theme-dark">
                        ...
                        <div className="text-lg whitespace-nowrap">Logout</div>
                    </a>
                </Link>
            </div>
        </div>
